...
func (u *Rider) GetByID() error {
	row := sqldb.QueryRow("SELECT * FROM rider WHERE id=$1", u.ID)
...
func (u *Rider) Get() error {
	if u.Email != "" {
		row := sqldb.QueryRow("SELECT id, fullname, password_ FROM rider WHERE email=$1", u.Email)
		err := row.Scan(&u.ID, &u.FullName, &u.Password)
		return err
	}
	row := sqldb.QueryRow("SELECT id, password_ FROM rider WHERE phonenumber=$1", u.Phonenumber)
...
func (u *Rider) Create() error {
	stmt, err := sqldb.Prepare(`INSERT INTO rider (id, fullname, email, 
...
func (u *Rider) UpdatePassword() error {
	_, err := sqldb.Exec("UPDATE rider SET password=$1 WHERE id=$2", u.Password, u.ID)
	return err
}

// Delete a rider from records
func (u *Rider) Delete() error {
	_, err := sqldb.Exec("DELETE * FROM rider WHERE id=$1", u.ID)
	return err
}
...
func GetAllRider() []Rider {
	riders := []Rider{}
	rows, err := sqldb.Query("SELECT * FROM rider")
	if err != nil {
